...
static void test_parse_array()
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < lept_get_array_size(lept_get_array_element(&v, i)); j++) {
            EXPECT_EQ_INT(LEPT_NUMBER,
                          lept_get_type(lept_get_array_element(lept_get_array_element(&v, i), j)));
            EXPECT_EQ_DOUBLE(
                j, lept_get_number(lept_get_array_element(lept_get_array_element(&v, i), j)));
        }
    }
    lept_free(&v);
}
